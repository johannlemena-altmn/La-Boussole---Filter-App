...
function buildPrompt(item, ctx) {
  const { projects, blocs, sources, weights } = ctx;

  const projectsBlock = projects.map(p =>
    `  - [${p.id}] ${p.name} — ${p.objective}${p.keywords ? ` (mots-clés : ${p.keywords})` : ""}`
  ).join("\n");

  const blocsBlock = blocs.map(b =>
    `  - ${b.code} · ${b.theme} — ${b.description}`
  ).join("\n");

  const sourcesBlock = sources.length
    ? sources.map(s => `  - "${s.name}" → ${s.protocol}`).join("\n")
    : "  (aucun protocole spécifique défini)";

  const weightsBlock = weights
    ? `Pondérations score sur 100 (la somme = 100) :
...
async function analyzeItem(item, ctx) {
  try {
    ...
    if (!parsed) {
      return { ok: false, error: "Réponse non parsable", raw };
    }
    // normalize
    ...
    return { ok: false, error: e.message || "Échec d'appel" };
  }
}
...
function exportAllMarkdown(items, ctx) {
  ...
  const head = [
    "# Le Filtre — Triage éditorial",
    "",
    `_Généré le ${new Date().toLocaleString("fr-FR")}_`,
    "",
    `**${done.length} contenu(s) qualifié(s)**`,
    "",
  ];
  // group by bloc
  const byBloc = {};
...
Object.assign(window, {
  buildPrompt, safeJSONParse, analyzeItem,
  scoreClass, exportItemMarkdown, exportAllMarkdown
});
